...
\noindent
A characterization of flatness
is that an $R$-module $M$ 
is flat if and only if
its torsion functors vanish.
That's the last proposition in this section.

I just wanted to highlight
Lemma \ref{lemma-long-exact-sequence-tor},
which says what happens in general with 
right exact sequences and tensor product:
as discussed in my introduction
to Section \ref{section-flat-morphisms},
the functor $-\otimes_R M$ is not left exact:
instead there are some Tor modules that appear.
So yes, it makes sense that the vanishing
of those modules is related to flatness.
...
in most cases one encounters in practice the existence of a birational morphism
