...
Riemann-Roch for $-K_C=T_C$ on a K3 surface,
$$
\chi(-K_C)=2+\frac{1}{2}(-K_C,-K_C)
$$
but $\chi(-K_C)=h^0(T_C)-h^1(T_C)$ and there are no holomorphic vector fields on
$C$ as long as it has genus $\geq 2$. To compute the self-intersection number
$(K_C,K_C)=(T_C,T_C)$ we may use the genus formula 
$2g-2=(\mathcal{O}(C),\mathcal{O}(C))$.
Here recall that since $C$ is smooth, $\mathcal{O}(C)$ is the normal bundle of
$C$ in $X$.
\end{enumerate}
\end{proof}
...
A {\it chamber} is a connected component of $C_x\setminus W(c_1,c_2)$.
\end{definition}
