...
{\bf Intento pessoal:}  considere a variação por geodésicas 
$$
...
$p' \in M$. Em conclusão: esse campo não está bem definido. (Se estivesse, com
...
$\gamma'(\ell)\neq -\sigma'(\ell)$. Isso significa que o vetor 
$\gamma'(\ell)-\sigma'(\ell)$ não é zero e portanto existe um vetor 
$V \in T_q M$ tal que $\left<\gamma'(\ell)-\sigma'(\ell),V\right>\neq 0$. 
Podemos supor que $\left<\gamma'(\ell)-\sigma'(\ell),V\right><0$, ou seja, 
podemos supor que um dos produtos $\left<V,\gamma'(\ell)\right>$,
$\left<V,\sigma'(\ell)\right>$ e maior do que o outro. Mas por enquanto não sei
como provar que podemos supor que os dois são negativos.

Agora usamos o fato de que $p$ e $q$ não são conjugados para levantar alguma
...
\begin{proof}
Para obter uma contradição, suponha que existe um punto $p \in M$ tal que
...
$\gamma'(\ell)=-\sigma'(\ell)$. Como no Exercício \ref{exercise-l8-3}, podemos
usar essas duas geodésicas para achar um loop geodésico de comprimento $2i(M)$.

Finalmente usamos o teorema de Synge \ref{theorem-Synge}, que nos diz que como
$M$ é de dimensão par, orientável, compacta e de curvatura positiva, o grupo
fundamental dela deve ser trivial. Isso significa que o laço geodésico que
achamos é homotópico a uma constante.

Isso não é possível: pois $\gamma$ e $\sigma$ são duas geodésicas minimizantes
ligando $p$ e $q$, e isso significa que 
\end{proof}
