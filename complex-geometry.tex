...
To prove that this extends to an isomorphism
of bundles all we have to do is show
that this is a biholomorphic map on the
total spaces of the bundles.
But holomorphicity is a local property,
and locally both bundles are isomorphic
because the exact sequence splits,
so the map above is trivial.

In local coordinates of the total spaces
$\coprod_{p \in M} \Lambda^{\text{rk}E'}E_p'$ and
$\coprod_{p \in M}\Lambda^{\text{rk}E''}E_p''$,
which have local frames given by a single local section,
the map above takes the coordinates to their product,
which is clearly holomorphic.
Thus this a holomorphic bunde isomorphism.
\end{proof}
