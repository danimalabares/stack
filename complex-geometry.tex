...
by assigning to $U_i$ the group of holomorphic sections of $O_Y(X)$.
...
corresponding to $O_Y(X)$ is locally
generated by $1/f_i$.
...
with base of dimension one we can construct a section of the bundle
$N_{X/Y}$.
\end{proposition}
...
establishes that this line bundle is in fact
...
\noindent
Notice that proportional sections of $N_{X/Y}$ 
may be constructed coming from the same deformation.
Thus we can consider an injective map whose
domain is the space of deformations of $X$ in $Y$
and codomain is $\mathbb{P}H^0(Y,N_{X/Y})$.
