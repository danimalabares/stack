...
+\sum_i\frac{\partial d}{\partial \overline{z}_i}d\overline{z}_i,
$$
...
\noindent
ChatGPT: psh is equivalent to the Hermitian matrix
$$
\left(\frac{\partial^2 f}{\partial z_i\partial \bar{z}_j}\right) 
$$
being positive semidefinite,
and it is also equivalent to its restriction to
...
$$
Now test it on a vector:
$$
dd^c\text{log}l(v,\bar{v})
=
\sqrt{-1}\sum
\left(\frac{|v|^2}{|z|^2}
-\frac{\bar{z}_iz_jv^i\bar{v}^j}{l^2}\right) 
$$
but the right-hand side
is just the usual Hermitian norm on $\mathbb{C}^n$
of the vector $(\bar{z}_iv^i)$,
so we apply Cauchy-Schwarz inequality to
show these functions must be positive.
And by the equality case in Cauchy-Schwarz we get nondegeneracy.
\end{proof}
