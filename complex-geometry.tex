...
Let $X\subset Y$ be complex manifolds.
There is a bijection between first order deformations
of $X$ in $Y$ and $H^0(X,N_{X/Y})$.
\end{proposition}

\begin{proof}
We start with a first order deformation and construct
a section of the normal bundle of $X$ in $Y$.
Let
$$
\xymatrix{
X\ar[r]\ar[d]
...
$$
be a first order deformation of $X$ in $Y$.
This means that $\mathcal{X}$ is a complex submanifold of
the product $Y \times B_\varepsilon$ 
and $\pi:\mathcal{X} \to B_\varepsilon$ is
a holomorphic submersion.

The first step is to describe the structure sheaf of $\mathcal{X}$.
For this, Voisin considers instead of the first order deformation,
a family $\mathcal{X} \to B$ for some complex manifold $B$ 
(of dimension 1! I don't know why it has to be 1).
Then by the inverse function theorem, we can find
open covers $V_i$ of $\mathcal{X}$
and $U_i$ of $X$ such that
$V_i \cap X= U_i$ for all $i$.
This is just using the submanifold
coordinates of the form
$(z^1,…,z^n,0,…,0)$ where $\dim X=n$.

But since  $\mathcal{X} \to B$ is a holomorphic
submersion, we apply the inverse function theorem
again to obtain for every $V_i \subset \mathcal{X}$ 
a biholomorphism from some $B_i \subset B$ containing $0$ to $V_i$.
This gives coordinates $(0,…,b^1,…,b^r)$ to $V_i$,
where $\dim B=r$.

This means that $V_i$ has local coordinates
$(z^1,…,z^n,0…,0,b^1,…,b^r)$ as long as $n+r<\dim \mathcal{X}$.
Or, as Voisin puts it, $V_i \cong U_i\times B_i$,
``$\mathcal{X} \to B$ is locally isomorphic to 
a product along $X$''.

Now we turn to the first order deformation,
which means restricting to the fat point subscheme
$B_\varepsilon$ of $B$. (Recall that this is
just the complex space whose topological space
is $0$ but its structure sheaf is $\mathcal{O}_B/(\varepsilon^2)$;
...
$$
\theta : 
\mathcal{O}_{X_\varepsilon|_{U_i}} \xrightarrow{\cong}
\mathcal{O}_{U_i}[\varepsilon]/(\varepsilon^2)
:= \mathcal{O}_{U_i} \otimes \mathbb{C}[\varepsilon]/(\varepsilon^2).
$$
This is the structure sheaf of the first order deformation.
Functions here look like $f+ \varepsilon g$ for 
$f,g \in \Gamma(U_i,\mathcal{O}_{U_i})$.

The functions in this sheaf must satisfy
some cocycle condition which essentially allows
us to glue everything back together.
Such a condition is what will allow us to construct
a global section of the normal sheaf.

We can find a great expression for the
functions in the sheaf $\mathcal{O}_{U_i}[\varepsilon](\varepsilon^2)$
using the exact sequence
$$
\xymatrix{
0\ar[r]
&\mathcal{O}_{U_i}\otimes \varepsilon \mathbb{C}\ar[r]
&\mathcal{O}_{U_i}[\varepsilon]/(\varepsilon^2)\ar[r]
&\mathcal{O}_{U_i}\ar[r]
&0
}
$$
which is just a fancy version of the
elementary dual number exact sequence
$$
\xymatrix{
0\ar[r]
&\varepsilon\mathbb{C}\ar[r]
&\mathbb{C}[\varepsilon]/(\varepsilon^2)\ar[r]
&\mathbb{C}\ar[r]
&0
}
$$
where $\mathbb{C}[\varepsilon]/(\varepsilon^2)$
are just expressions $a+\varepsilon b$ with $\varepsilon^2=0$,
so that the first arrow is the inclusion $\varepsilon b \mapsto  \varepsilon b$
and the second one the projection $a+\varepsilon b \mapsto  a$.

The sheaf sequence splits and we conclude that functions
of the deformation space $\mathcal{X}$ locally look
like $f+\varepsilon g$ for $f,g \in H^0(U_i,\mathcal{O}_{U_i})$.
...
\begin{proof}
My answer as of December 28, 2025,
is: use \cite[Proposition 3.2.1(ii) or (ii) p.50]{Sernesi-deformations}
to argue that the first order deformations
of $C$ inside $X$ are parametrized by the
sections of the normal bundle $H^0(C,N_{C/M})$.

Then argue why we have to remove that 1,
i.e. why we have to projectivize,
i.e. why these deformations are in the linear system $|C|$.

Then show that $\dim H^0(C,N_{C/M})=g+1$.

And then… show that the deformation space
of a curve (in a K3?) is unobstructed and in
fact any infinitesimal deformation can be integrated
to a full deformation. But in fact
this is probably not necessary since the question
is asking to show that $\dim \mathcal{X} \leq  g$
and not to compute the exact value.

Let $B$ be a connected complex manifold,
$\mathcal{X}\subset M\times B$ and
$\mathcal{X} \to B$ a holomorphic submersion
with fibre $C$ at a distinguished point  $0 \in B$.
All this just says that $\mathcal{X}$ is a deformation
of $C$ in $M$.

As for how to compute $h^1(T_C)$, the only thing
that makes sense is using the normal bundle exact sequence.
This gives a long exact sequence in cohmology,
$$
\xymatrixcolsep{1em}
\xymatrix{
0\ar[r]&
H^0(T_C)\ar[r]&H^0(T_X|_C)\ar[r]&H^0(N)\ar[r]&H^1(T_C)\ar[r]&H^1(T_X|_C)\ar[r]&
H^1(N)\ar[r]&0
}
$$
and then I just need to compute neighbouring terms of
$H^1(T_X)$, namely $H^0(N)$ and $H^1(T_X|_C)$.
In fact, an old computation shows that $h^0(N)=g+1$.
About $H^1(T_X|_C)$ I'm not sure…
Notice that if we want $h^1(T_C)\leq g$
we need the map $H^0(N)\to H^1(T_C)$ to have kernel,
so $H^0(T_X|_C)$ cannot be zero!


Other notes. Isn't the restricted tangent bundle 
just the pullback bundle by the inclusion?
At every point we put the holomorphic tangent space
of the K3 surface?
Also see \href{https://en.wikipedia.org/wiki/Teichm%C3%BCller_space}{wiki}
on Teichmüller space for some remarks about the complex structure approach.
\end{proof}


When considering the canonical map $V \otimes V^* \simeq \text{End}(V)$,
there's also the trace ideal. Would this help at all?

\section{Kähler cone}
\label{section-kahler-cone}
