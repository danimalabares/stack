...
\section{Analytic set}
\label{section-analytic-set}
...
\end{reference}
An {\it analytic set} or {\it variety} $Y$ of a complex manifold $X$ is a
...
We shall follow Demailly to prove
directly that if $I$ is prime then $I(V(I))=I$.
But we need a bunch of results before.
...
\begin{proof}
Lemma \ref{lemma-integral-extension}
...
$$
f^r+b_{r-1}f^{r-1}+\ldots+b_1f+b_0=0\qquad \text{mod }\mathcal{I}.
$$
Since $f$ vanishes on $(A,0)$,
this says that $b_0$ vanishes in $(A,0)$ too.
(Right?)
