...
The global version of Theorem \ref{theorem-Toponogov-local} is the following.
Notice it coincides with our intuition from comparing plane triangles with
spherical ones; spherical are fatter.

\begin{theorem}[Toponogov, hinge]
\label{theorem-toponogov-hinge}
$M$ complete, $K \geq k$. $\gamma_1,\gamma_2$ normalized geodesics with
$\gamma_1(0)=\gamma_2(0)$. Suppose $\gamma_1$ is minimizing. If $k>0$ suppose
additionally that $\ell(\gamma_2) \leq \pi/\sqrt{k}$ (this is where conjugate
points appear).
...
\section{Preisman Theorem}
\label{section-Preisman-theorem}
...
\begin{theorem}[Preisman]
\label{theorem-Preisman}
...
Let $M^n$ be a complete manifold with $K\leq 0$. Suppose that every element of
...
\label{section-Busemann-functions}
...
\label{theorem-Cheeger-Gromoll}
Let $M$ be complete with  $\text{Ric}\geq0$. If $M$ has a line, then $M$ is
isometric to $N\times\mathbb{R}$ for a compact manifold $N$.
\end{theorem}
