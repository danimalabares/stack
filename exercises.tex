...
\end{enumerate}
Describe your space clearly and succinctly explain why there
...
\dim_k {\mathfrak m}/{\mathfrak m}^2 = 2.
$$
Notations: $f$, $F$, $x, y\in {\mathfrak m}$, $I$ as in
Exercise \ref{exercise-embedding-2-dim-1}. Please
...
Hint: Choose an auxiliary invertible module $\mathcal{L}$
...
\section{Algebraic Geometry, Final Exam, Spring 2022}
\label{section-final-exam-spring-2022}

\noindent
These were the questions in the final exam of a course on Algebraic Geometry,
in the Spring of 2022 at Columbia University.
...
\begin{enumerate}
\item a {\it scheme},
\item a {\it quasi-coherent module} on a scheme $X$,
\item a {\it flat} morphism of schemes $X \to Y$,
\item a {\it finite} morphism of schemes $X \to Y$,
\item a {\it group scheme} $G$ over a base scheme $S$,
\item a {\it family of varieties} over a base scheme $S$,
\item the {\it degree} of a closed point $x$ on a variety $X$
over the field $k$,
\item the usual {\it logarithmic height} of a point
$p = (a_0 : \ldots : a_n)$ in $\mathbf{P}^n(\mathbf{Q})$, and
\item a {\it $C_i$ field}.
\end{enumerate}
\end{exercise}
...
one of them).
\begin{enumerate}
\item morphisms from a scheme $X$ to the affine scheme $\Spec(A)$,
\item cohomology of a quasi-coherent module $\mathcal{F}$ on
an affine scheme $X$,
\item the Picard group of $\mathbf{P}^1_k$ where $k$ is a field,
\item the dimensions of fibres of a flat proper morphism
$X \to S$ for $S$ Noetherian,
\item $\mathbf{G}_m$-equivariant modules on a scheme $S$, and
\item Bezout's theorem on intersections (restrict to
...
Let $F \in \mathbf{C}[T_0, \ldots, T_n]$ be homogeneous of degree $3$.
Given $3$ vectors $x, y, z \in \mathbf{C}^{n + 1}$ consider the condition
$$
(*)\quad
F(\lambda x + \mu y + \nu z) = 0 \text{ in } \mathbf{C}[\lambda, \mu, \nu]
$$
\begin{enumerate}
\item What is the dimension of the space of all choices of $x, y, z$?
\item How many equations on the coordinates of $x$, $y$, and $z$
is condition (*)?
\item What is the expected dimension of the space of all triples
$x, y, z$ such that (*) is true?
\item What is the dimension of the space of all triples such that
$x, y, z$ are linearly dependent?
\item Conclude that on a hypersurface of degree $3$ in $\mathbf{P}^n$
we expect to find a linear subspace of dimension $2$ provided
$n \geq a$ where it is up to you to find $a$.
\end{enumerate}
\end{exercise}

\begin{exercise}[Heights]
\label{exercise-heights}
Let $K$ be a field. Let $h_n : \mathbf{P}^n(K) \to \mathbf{R}$, $n \geq 0$
be a collection of functions satisfying the $2$ axioms we discussed
in the lectures. Let $X$ be a projective variety
over $K$. Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module
and recall that we have constructed in the lectures an associated
height function $h_\mathcal{L} : X(K) \to \mathbf{R}$.
Let $\alpha : X \to X$ be an automorphism of $X$ over $K$.
\begin{enumerate}
\item Prove that $P \mapsto h_\mathcal{L}(\alpha(P))$
differs from the function $h_{\alpha^*\mathcal{L}}$
by a bounded amount. (Hint: recall that if there is a morphism
$\varphi : X \to \mathbf{P}^n$ with
$\mathcal{L} = \varphi^*\mathcal{O}_{\mathbf{P}^n}(1)$, then by construction
$h_\mathcal{L}(P) = h_n(\varphi(P))$ and play around with that.
In general write $\mathcal{L}$ as a difference of two of these.)
\item Assume that $h_\mathcal{L}(P) - h_\mathcal{L}(\alpha(P))$
is unbounded on $X(K)$. Show that
$h_\mathcal{N}$ with
$\mathcal{N} = \mathcal{L} \otimes \alpha^*\mathcal{L}^{\otimes -1}$
is unbounded on $X(K)$.
\item Assume $X$ is an elliptic curve and that
$\mathcal{L}$ is a symmetric ample invertible module on $X$
such that $h_\mathcal{L}$ is unbounded on $X(K)$. Show
that there exists an invertible module $\mathcal{N}$ of degree $0$
such that $h_\mathcal{N}$ is unbounded.
(Hints: Recall that $X$ is an abelian variety of dimension $1$.
Thus $h_\mathcal{L}$ is quadratic up to a constant by results
in the lectures. Choose a suitable point $P_0 \in X(K)$.
Let $\alpha : X \to X$ be translation by $P_0$. Consider
$P \mapsto h_\mathcal{L}(P) - h_\mathcal{L}(P + P_0)$.
Apply the results you proved above.)
\end{enumerate}
\end{exercise}

\begin{exercise}[Monomorphisms]
\label{exercise-monomorphisms}
Let $f : X \to Y$ be a monomorphism in the category of schemes:
for any pair of morphisms $a, b : T \to X$ of schemes if
$f \circ a = f \circ b$, then $a = b$. Show that $f$ is injective
on points. Does you argument say anything else?
\end{exercise}
...
Let $k$ be an algebraically closed field.
\begin{enumerate}
\item If $G = \mathbf{G}_{m, k}$ show that if $G$ acts on a
projective variety $X$ over $k$, then the action has a fixed point, i.e.,
prove there exists a point $x \in X(k)$ such that $a(g, x) = x$ for
all $g \in G(k)$.
\item Same with $G = (\mathbf{G}_{m, k})^n$ equal to the product of
$n \geq 1$ copies of the multiplicative group.
\item Give an example of an action of a connected group scheme $G$
on a smooth projective variety $X$ which does not have a fixed point.
\end{enumerate}
\end{exercise}
